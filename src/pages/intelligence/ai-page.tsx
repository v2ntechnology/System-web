...
export default function AIPage() {
  ...
  return (
    <div>
      {/* `bleed={false}`: sem fileira de números embaixo, a faixa não precisa do
          respiro que existe para os cards a morderem. Conversa não tem número de
          abertura: o que importa aqui é o que está escrito. */}
      <PageHero
        title="IA RookHub"
      ...
      <PagePanel className="mt-6">
        <PlanGuard module="ai">
          <div className="grid gap-4 lg:grid-cols-[260px_1fr]">
            <LightCard className="hidden lg:block">
              <div className="space-y-3">
                ...
                <p className="text-on-light-muted px-1 text-xs font-semibold uppercase tracking-wide">
                  Histórico
                </p>
                <ul className="space-y-1">
                  {HISTORY.map((item) => (
                    <li key={item}>
                      <button
                        type="button"
                        className="text-on-light-muted hover:bg-light-container hover:text-on-light w-full truncate rounded-md px-2 py-2 text-left text-sm transition-colors"
                      >
                        {item}
                      </button>
                    </li>
            ...
            <LightCard className="flex h-[70vh] flex-col !p-0">
                  ...
                  <div className="space-y-4">
                    ...
                    {thinking && (
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <BotIcon className="h-4 w-4 animate-pulse text-primary" />A IA está
                        analisando os dados…
                      </div>
                    )}
                  </div>

...
} from '@/management/features/maintenance/components/mechanical-alerts-queue';
import { FleetMaintenanceDueQueue } from '@/management/features/maintenance/components/fleet-maintenance-due-queue';
...
export function NotificationsPage() {
  ...
  const abaParam = params.get('aba')?.toUpperCase();
  const aba: AbaId = abaParam === 'SEGURANCA' || abaParam === 'PENDENCIAS' ? abaParam : 'TODOS';
    ...
    () =>
      aba === 'TODOS'
        ...
        : notifications.filter(
            (item) => item.source === (aba === 'SEGURANCA' ? 'SAFETY' : 'MAINTENANCE'),
          ),
    [notifications, aba],
  );
  ...
  const counts = useMemo(() => {
    const base =
      source === TODAS_ORIGENS
        ? notificationsDaAba
        : notificationsDaAba.filter((item) => item.source === source);
  ...
  return (
    <>
      <HeroBand
        ...
        description="O que pede atenção agora: segurança, pendências de manutenção e avisos da operação."
      >
            ...
            tabs={ABAS.map((item) => ({
              ...
              count:
                item.id === 'TODOS'
                  ...
                  : item.id === 'SEGURANCA'
                    ? notifications.filter((notification) => notification.source === 'SAFETY')
                        .length
                    : notifications.filter((notification) => notification.source === 'MAINTENANCE')
                        .length,
            }))}
            value={aba}
            onValueChange={(next) => setParams(next === 'TODOS' ? {} : { aba: next.toLowerCase() })}
            label="Categorias de alerta"
          >
            <AlertTabIntro aba={aba} />
            {aba === 'PENDENCIAS' ? <MaintenanceDueSection /> : null}
            {aba === 'PENDENCIAS' ? <MaintenanceCanSection /> : null}

            {!(aba === 'PENDENCIAS' && notificationsDaAba.length === 0) ? (
              <div className="mt-8 border-light-outline border-t pt-8">
                <QueryState isPending={isPending} isError={isError} label="as notificações">
                  <>
                    ...
                    <div className="bg-on-light/[0.025] mb-6 flex flex-wrap items-center gap-x-4 gap-y-3 rounded-xl p-3 sm:p-4">
                      {/*
                        ...
                        {visible.map((item) => {
                          ...
                          return (
                            <li
                              key={item.id}
                              className={cn(
                                'bg-light-container border-light-outline hover:border-primary-on-light/25 rounded-xl border p-4 transition-colors sm:p-5',
...
function AlertTabIntro({ aba }: { aba: AbaId }) {
  const copy = {
    TODOS: {
      title: 'Priorize o que exige decisão',
      description: 'Uma fila única para os avisos que chegam de toda a operação.',
    },
    SEGURANCA: {
      title: 'Segurança da frota',
      description: 'Eventos de condução e situações que pedem tratativa antes da próxima viagem.',
    },
    PENDENCIAS: {
      title: 'Pendências da frota',
      description: 'Vencimentos e sinais mecânicos que precisam ser avaliados por veículo.',
    },
  }[aba];

  return (
    <header className="bg-on-light/[0.035] border-light-outline mb-8 flex flex-wrap items-start justify-between gap-3 rounded-xl border px-4 py-4 sm:px-5">
      <div>
        <h2 className="text-on-light text-body-lg font-semibold">{copy.title}</h2>
        <p className="text-on-light-muted text-body-sm mt-1">{copy.description}</p>
      </div>
      <span className="bg-light text-on-light-variant text-label-md rounded-full px-3 py-1.5 normal-case">
        Atualizado pela última coleta
      </span>
    </header>
  );
}

function MaintenanceDueSection() {
  ...
  return (
    <section className="bg-light-container border-light-outline mb-6 rounded-xl border p-5 sm:p-6">
      <QueryState
...
/** Alertas CAN são sinais para triagem; manutenção é quem executa o trabalho. */
function MaintenanceCanSection() {
  const { data, isPending, isError } = useQuery({
    queryKey: ['notifications', 'alertas-mecanicos', 30],
    queryFn: () => fetchMechanicalAlerts(30),
  });
  const [tipo, setTipo] = useState<string | null>(null);
  const resumo = useMemo(() => aggregateMechanicalAlerts(data ?? []), [data]);
  const veiculos = useMemo(
    () =>
      tipo === null
        ? resumo.veiculos
        : resumo.veiculos.filter((veiculo) =>
            veiculo.tipos.some((alerta) => alerta.description === tipo),
          ),
    [resumo.veiculos, tipo],
  );

  return (
    <section
      aria-labelledby="alertas-manutencao"
      className="bg-light-container border-light-outline rounded-xl border p-5 sm:p-6"
    >
      <div className="border-light-outline mb-5 flex flex-wrap items-baseline justify-between gap-3 border-b pb-5">
        <div>
          <h2
            id="alertas-manutencao"
            className="font-sora text-on-light text-headline-md tracking-[-0.02em]"
          >
            Manutenção · alertas da rede CAN
          </h2>
          <p className="text-on-light-muted text-label-md mt-1 normal-case">
            Sinais agrupados por veículo nos últimos 30 dias; não são diagnóstico de oficina.
          </p>
        </div>
        <span className="text-on-light-muted text-label-md tabular normal-case">
          {resumo.total.toLocaleString('pt-BR')} ocorrências · {resumo.veiculos.length} veículos
        </span>
      </div>

      <QueryState isPending={isPending} isError={isError} label="os alertas mecânicos">
        {resumo.porTipo.length > 0 ? (
          <div className="mb-6">
            <p className="text-on-light-variant text-label-md mb-3 normal-case">
              Por tipo de alerta
            </p>
            <MechanicalKindFilters porTipo={resumo.porTipo} selected={tipo} onSelect={setTipo} />
          </div>
        ) : null}
        <MechanicalAlertsQueue
          veiculos={veiculos}
          note={
            tipo
              ? `Filtro: ${tipo.toLocaleLowerCase('pt-BR')}.`
              : 'Escolha um tipo acima para concentrar a triagem.'
          }
        />
      </QueryState>
    </section>
